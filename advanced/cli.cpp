...
void stdin_cb(ev::io& w, int revent)
{
	...
	(*reinterpret_cast<ev::socket*>(w.data))<<buf;
}
//}}}
...
void socket_cb(ev::socket& w, int revent)
{
	...
	else
	{
		...
		if((rc = write(w.fd, buf.c_str(), buf.length()) != static_cast<int>(buf.length())))
		{
			if( rc > 0 )
			{
				std::cout<<"String was written only partially."<<std::endl;
			}
			else
			{
